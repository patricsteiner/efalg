package ch.fhnw.edu.efalg.chessboard.gui;
...
import ch.fhnw.edu.efalg.chessboard.algorithms.KnightsTourNoHeuristic;
import ch.fhnw.edu.efalg.chessboard.algorithms.KnightsTourWarnsdorf;
import ch.fhnw.edu.efalg.chessboard.algorithms.KnightsTourWarnsdorfOpposite;
import ch.fhnw.edu.efalg.chessboard.impl.GraphicalBoardImpl;
...
public final class ChessBoardGUI {
	...
	private JMenuBar createMenuBar(final StatusHolder holder) {
		...
		for (int size : new int[] { 4, 6, 8, 10, 12, 18, 32, 64, 100 }) {
			JMenuItem item = new JMenuItem(new NewAction(holder, size, size));
			item.setText(size + "/" + size);
			newMenu.add(item);
		}
		newMenu.addSeparator();
		int[] rowSizes = new int[] { 4, 8, 20, 60 };
		int[] columnSizes = new int[] { 6, 10, 30, 100 };
		...
		JMenuItem noheuristic = new JMenuItem(new AlgorithmExecutionAction(holder, KnightsTourNoHeuristic.class));
		noheuristic.setText("Knight's Tour No Heuristic");
		noheuristic.setMnemonic(KeyEvent.VK_N);
		algorithms.add(noheuristic);
		...
		JMenuItem warnsdorfOpposite = new JMenuItem(new AlgorithmExecutionAction(holder, KnightsTourWarnsdorfOpposite.class));
		warnsdorfOpposite.setText("Knight's Tour Warnsdorf Opposite");
		warnsdorfOpposite.setMnemonic(KeyEvent.VK_O);
		algorithms.add(warnsdorfOpposite);
