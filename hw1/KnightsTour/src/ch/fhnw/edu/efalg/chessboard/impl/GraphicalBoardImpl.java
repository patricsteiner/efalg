...
import ch.fhnw.edu.efalg.chessboard.GraphicalBoard;
import ch.fhnw.edu.efalg.chessboard.Position;
import ch.fhnw.edu.efalg.chessboard.StatusHolder;
import ch.fhnw.edu.efalg.chessboard.gui.ChessBoardClickDetector;
import ch.fhnw.edu.efalg.chessboard.gui.ChessBoardFieldUpdater;
...
public final class GraphicalBoardImpl extends JPanel implements GraphicalBoard {
	...
	public void init(final ChessBoard newBoard) {
		...
		for (int i = 0; i < numOfRows; ++i) {
			for (int j = 0; j < numOfColumns; ++j) {
				...
				newBoard.addChessBoardListener(updater);
	...
	public synchronized void paint(final Graphics g) {
		...
		for (Line line : lines) {
			...
			//graphics.drawString(new Integer(i++).toString(),startX - DOT_SIZE*7, startY - DOT_SIZE*3);
		}
